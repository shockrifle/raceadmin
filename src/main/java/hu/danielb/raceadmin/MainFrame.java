...
import hu.danielb.raceadmin.components.GenericTabbedPane;
import hu.danielb.raceadmin.components.PrintHeaderMenuItem;
import hu.danielb.raceadmin.components.table.AttributiveCellTableModel;
import hu.danielb.raceadmin.components.table.CellSpan;
import hu.danielb.raceadmin.components.table.MultiSpanCellTable;
import hu.danielb.raceadmin.data.*;
...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void loadTeams(String sex, AgeGroup ageGroup, Vector<Vector<String>> data) {
        ...
        List<hu.danielb.raceadmin.data.Team> teams = makeTeams(data);
    ...
    private List<hu.danielb.raceadmin.data.Team> makeTeams(Vector<Vector<String>> data) {
        HashMap<String, hu.danielb.raceadmin.data.Team> tmp = new HashMap<>();
        for (Vector<String> data1 : data) {
            ...
            do {
                ...
                if (tmp.containsKey(sch)) {
                    ...
                    tmp.put(sch, new hu.danielb.raceadmin.data.Team(sch));
                    tmp.get(sch).addMember(new Contestant(
                            Integer.parseInt(data1.get(0)),
                            Integer.parseInt(data1.get(1)),
                            data1.get(3),
        ...
        List<hu.danielb.raceadmin.data.Team> teams = tmp.entrySet().stream().filter(entry -> entry.getValue().isFull()).map(Map.Entry::getValue).collect(Collectors.toList());
        ...
        do {
            ...
            for (int i = 1; i < teams.size(); i++) {
                if (teams.get(i).lowerThan(teams.get(i - 1))) {
                    hu.danielb.raceadmin.data.Team t = teams.get(i - 1);
    ...
    public class Team {
        ...
        String name = "";
        List<hu.danielb.raceadmin.data.Team> teams = new ArrayList<>();
